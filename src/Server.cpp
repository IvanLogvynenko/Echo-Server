...
std::string server_client::Server::awaitNewMessage(Connection* conn) {
	...
	try {
		...
		logger << Logger::important << "Connection closed by client" << std::endl;
        removeSocket(conn);
        throw e;
	}
...
std::thread *server_client::Server::startMessageIncomeHandlingThread(
	...
	return new std::thread([this, conn, on_recieve]() {
		...
		while (!this->message_handler_stop_trigger) {
			try {
				...
				logger << Logger::important << "Connection closed by client => stopping thread" << std::endl;
				break;
			}
		}
	});
...
void server_client::Server::removeSocket(Connection *conn) {
	...
	logger << "Removing message income handling thread for connection " << *conn << std::endl;
	std::lock_guard<std::mutex> other_lock(message_handlers_lock);
	income_message_handlers[*conn]->join();
	delete income_message_handlers[*conn];
